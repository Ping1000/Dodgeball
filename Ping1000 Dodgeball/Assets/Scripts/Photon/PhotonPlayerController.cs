...
public class PhotonPlayerController : MonoBehaviour
{
    ...
    private Queue<PhotonCharacterAction> actionsQueue;
    ...
    private PhotonTeamController teamController;
    ...
    {
        teamController = GetComponentInParent<PhotonTeamController>();
        photonView = GetComponent<PhotonView>();
        actionsQueue = new Queue<PhotonCharacterAction>();
                ...
                switch (selectedAction)
                {
                    case ActionType.Move:
                        if (hit.collider.CompareTag(floorTag))
                        {
                            ...
                            actionsQueue.Enqueue(new PhotonCharacterAction(selectedAction, _mover, hit.point));
    ...
    IEnumerator ExecutingActions()
    {
        ...
        {
            PhotonCharacterAction action = actionsQueue.Dequeue();
            action.DoAction();
            yield return new WaitUntil(() => !(action.isActing));
        }

        // reset variables when done
        actionsQueue = new Queue<PhotonCharacterAction>(); // should be garbage collected, right? // God I hope so
        canBuildActions = false;
        isBuilding = false;
        isWaiting = false;
        isActing = false;
